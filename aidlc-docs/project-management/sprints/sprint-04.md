...
- [x] **S-0105** Cấu hình Skill (instructions/answer policy) qua progressive disclosure · [[units-retrieval-answer#UoB-01: Answer Policy Question]]
- [x] **S-0702** Chọn provider/model per skill mà không sửa code · [[units-governance-ops#UoB-07: Multi-Provider / Model Configuration]]
- [x] **S-0503** Xem token usage & monitoring · [[units-governance-ops#UoB-05: Admin Portal / Monitoring]]
...
- 2026-06-10: S-0105, S-0702, S-0503 implemented. Created robust tests for `ProvidersAdminController`, `SkillsController`, and a full integration test `SkillAndProviderIntegrationTests` verifying that cache invalidation triggers routing changes without redeployment. Sprint 04 verification is complete.
...
- [x] Code review pass.
- [x] Unit test cho admin CRUD, re-index trigger, skill registry, provider route resolution.
- [x] Integration test: HR đổi Skill/provider config → có hiệu lực mà không deploy lại.
- [x] Tài liệu kỹ thuật ở `tasks/` được link và cập nhật.
- [x] Skill/persona config KHÔNG override security trimming, citation hay deny-by-default ([[units-retrieval-answer#UoB-01: Answer Policy Question]] §11.5); secret theo Key Vault, cấm raw API key trong UI.
